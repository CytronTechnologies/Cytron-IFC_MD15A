...
//	Project description	: Sample2 for IFC-MD15A with Control Panel
//  Date				: 21 April 2008
//
//=====================================================================================
	...
	while(1)
	{
		...
		if(cp1_sw(1)==0)			// Test whether SW1 on IFC-CP04 is pressed
		{
			...
			while(1)
			{
				...
				cp1_dec(md_spval(add_md1),3);	// send message to be displayed by calling function cp1_str
		...
		if(cp1_sw(2)==0)				// Test whether SW2 on IFC-CP04 is pressed
		{
			...
			{				
				...
				cp1_dec(md_spval(add_md1),3);		// send message to be displayed by calling function cp1_str
				cp1_goto(1,10);						// set the position of message to be displayed
				cp1_dec(md_enval(add_md1),5);
				md_cw(add_md1);						// Run motor in clockwise
				md_speed(add_md1,250);				// motor's speed = 250
				led3=1;
				md_encon(add_md1, 20, 4, 100);	// change motor speed and direction
		...
		if(cp1_sw(3)==0)			   		// Test whether SW3 on IFC-CP04 is pressed
		{
			...
			while(1)
			{
				...
				cp1_str("Speed:");			// send message to be displayed by calling function cp1_str
				cp1_goto(1,6);				// set the position of message to be displayed
				cp1_dec(speed,3);			// send message to be displayed by calling function cp1_str
